...
Future<Response> onRequest(RequestContext context) async {
  ...
  final result = StockValidators.resetSchema.tryParse(parsedBody.body!);
  if (!result.success) {
    return Response.json(
  ...
  const resetNote = 'Stock reset to zero due to wastage.';
  ...
  try {
    final movementResult = await StockMovementService.apply(
      ...
      reason: StockChangeReason.WASTAGE,
      quantity: 0,
      note: resetNote,
      resetAll: true,
    );
    ...
    return Response.json(
      ...
      body: {
        'status': 'error',
        'message': 'Unable to reset stock at the moment.',
      },
    );
  } on Exception {
    return Response.json(
