...
import '../../../../routes/cashier/categories/index.dart' as route;
...
void main() {
  group('POST /cashier/categories', () {
    test('responds with a 405 when method is not allowed', () async {
      final context = _MockRequestContext();
      when(() => context.request).thenReturn(
        Request.post(Uri.parse('http://127.0.0.1/cashier/categories')),
      );
  ...
  group('GET /cashier/categories', () {
    test(
      'returns 401 UNAUTHORIZED when storeId is missing in token',
      () async {
        final context = _MockRequestContext();
        when(() => context.request).thenReturn(
          Request.get(Uri.parse('http://127.0.0.1/cashier/categories')),
        );
    ...
    test('returns 400 INVALID_INPUT for malformed isActive', () async {
      final context = _MockRequestContext();
      when(() => context.request).thenReturn(
        Request.get(
          Uri.parse('http://127.0.0.1/cashier/categories?isActive=maybe'),
        ),
      );
      when(() => context.read<TokenPayload>()).thenReturn(
        TokenPayload(
    ...
    test('returns 200 with categories list', () async {
      ...
      when(() => context.request).thenReturn(
        Request.get(
          Uri.parse('http://127.0.0.1/cashier/categories?isActive=true'),
        ),
      );
      when(() => context.read<TokenPayload>()).thenReturn(
        TokenPayload(
